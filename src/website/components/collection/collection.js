...
class SiteCollection extends SiteLayoutContainerBase {
  static properties = {
    ...super.properties,
  ...
  shouldForceCollectionEditingFromEvent(event) {
    ...
    for (const node of path) {
      ...
      if (tagName.startsWith("site-") && tagName !== "site-collection") {
        return true;
      }
    }
  ...
  getCollectionRenderedChildNodes() {
    ...
    // Avoid recursive ownership checks that depend on getChildNodes().
...
class OwbCollection extends LitElement {
  static styles = [
    css`
      :host {
        display: block;
...
if (!customElements.get("site-collection")) {
  customElements.define("site-collection", SiteCollection);
}
